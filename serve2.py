...
def sc(i,normalize=True,summarization=True,pdb=None):
    ...
    #classifier = pipeline("sentiment-analysis",model = "cross-encoder/ms-marco-MiniLM-L-12-v2")
    ...
    if a=='POSITIVE':
        return classifier(text)[0]['score'] , 1
    else:
        return 1-classifier(text)[0]['score'] , 0
...
def sentiment_rank():
    ...
    pdb = get_papers()
    pids=list(pdb.values())
    

    print(list(pdb.values()))

    predict_labels=[sc(i=i,normalize=True,summarization=True,pdb=pdb)[1] for i in range(len(pids))]
    ...
    scores=[sc(i=i,normalize=True,summarization=True,pdb=pdb)[0] for i in range(len(pids))]
    ...
    predict_labels=[sc(i=i,normalize=True,summarization=True,pdb=pdb)[1] for i in range(len(predict_labels)) if i not in indexes]
    ...
    print(classification_report(df2['target'], df2['predict_labels']))
